    ...
    if file_path:
        for file_name in os.listdir(file_path):
            if os.path.isfile(os.path.join(file_path, file_name)):
                ...
                file_extension_folder = os.path.join(file_path, file_extension)
                if not os.path.exists(file_extension_folder):
                    os.makedirs(file_extension_folder)
                os.rename(os.path.join(file_path, file_name), os.path.join(file_extension_folder, file_name))
...
lbl_heading = Label(root, text="File Manager App",font=("bold",18))
...
exit_btn.grid(row=3, column=3, pady=20, padx=20)
...
folder_entry.grid(row=0, column=1, padx=5, pady=5, sticky=W+E)
...
organize_button.grid(row=0, column=3, pady=20, padx=20, sticky=W)
